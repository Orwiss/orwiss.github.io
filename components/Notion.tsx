...
export default function Projects() {
  ...
  const filteredData = filter ? data.filter((item) => item.properties?.["카테고리"].multi_select.some((tag: any) => tag.name === filter)) : data;
        ...
        {selectedPage ? (
          ...
          <div>
            <div className="flex flex-col sm:flex-row gap-4 my-10 xl:my-20">
              {Array.from(new Set(data.flatMap(item => item.properties?.["카테고리"].multi_select.map((tag: any) => tag.name)))).reverse().map((tag) => (
                <button
                  key={tag}
